...
const NoteForm = ({allNotes,setAllnotes}) => {
    const { values, errors, handleChange, touched, handleBlur, handleSubmit, setFieldValue } = useFormik({
        ...
        onSubmit: (values, action) => {
            ...
            let newNote = { title: values.userNote, id: randomId, name: values.userName, email: values.userEmail, password: values.userPassword }
    ...
    return (
        <form className='mx-3 mt-4 form' onSubmit={handleSubmit} >
            ...
            {/* <DateRangePicker block name='daterange' className='mt-3' disabledDate={beforeToday()} onChange={handleChange} onBlur={handleBlur} />
                {errors.daterange && touched.daterange ? <p className='text-white' > {errors.daterange} </p> : null} */}
            {/* Password input */}
            ...
            <Select options={options} name='food' onChange={(e) => { setFieldValue("food", e.value) }} onBlur={handleBlur} className='mt-3' />
